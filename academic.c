...
struct student_t find_student_by_id(struct student_t *_students, unsigned short int _student_size, char *_id)
{
    struct student_t std;
    for(int i = 0; i<_student_size;i++){
        if ( strcmp(_id , _students[i].id ) == 0){
            strcpy(std.id , _students[i].id);
            strcpy(std.name , _students[i].name);
            strcpy(std.year , _students[i].year);
            strcpy(std.study_program , _students[i].study_program);
            std.gpa = _students[i].gpa;
...
struct course_t find_course_by_code(struct course_t *_courses, unsigned short int _course_size, char *_code)
{
    struct course_t crs;
    for(int i = 0; i<_course_size;i++){

        if ( strcmp(_code , _courses[i].code ) == 0){
            strcpy(crs.code , _courses[i].code);
            strcpy(crs.name , _courses[i].name);
            crs.credit = _courses[i].credit;
            crs.passing_grade = _courses[i].passing_grade;
            break;
        }else continue;
    }
